	...
	function send_dijkstra() {
		...
		$.ajax({
			...
			success: function(response) {
				...
				$sqlnod = "SELECT * FROM rs ";
				$run_nod = mysqli_query($koneksi, $sqlnod);
				...
				const split_nod = nod.split("->");
				console.log(split_nod.length);
				var nm_rs = new Array(14);
				// <?php while ($fetch1 = mysqli_fetch_array($run_nod, MYSQLI_ASSOC)) { ?>
				// 	nm_rs[<?= $fetch1['id'] - 1 ?>] = <?= $fetch1['tujuan'] ?>;
				// <?php } ?>
				nm_rs[0] = "RS St. CAROLUS BORROMEUS";
				nm_rs[1] = "RS Jiwa Naimata";
				nm_rs[2] = "Rumah Sakit AURI";
				nm_rs[3] = "RSU LEONA";
				nm_rs[4] = "Rumah Sakit Ibu dan Anak Dedari";
				nm_rs[5] = "RS.Kartini";
				nm_rs[6] = "Siloam Hospitals";
				nm_rs[7] = "RSU. Mamami";
				nm_rs[8] = "Rumah Sakit Umum Daerah S.K. LERIK";
				nm_rs[9] = "Rumah Sakit Bhayangkara";
				nm_rs[10] = "RSU UNDANA";
				nm_rs[11] = "RSUD Prof. Dr. W Z Johannes";
				nm_rs[12] = "Rumah Sakit Wirasakti";
				nm_rs[13] = "RSAL Samuel J. Moeda";

				var text_nod = "";
				var tmp_jarak = 0;
				var abc = "";
				for (var i = 0; i < split_nod.length; i++) {

					if (split_nod[i] == 20 && i > 0) {
						if (split_nod[i - 1] == 11) {
							split_nod[i] = 12;
						} else if (split_nod[i - 1] == 10) {
							split_nod[i] = 11;
						}
						for (var j = 0; j < split_nod.length; j++) {
							if (split_nod[i] == split_nod[j] && i != j) {
								if (split_nod[i - 1] == 10) {
									split_nod[i] = "";
								}
							}
					...
					var cba = "";
					if (i >= split_nod.length - 1) {
						if (split_nod[i] != nilai_des - 1) {
							cba = "";
							split_nod[i] = split_nod[i - 1];
						} else {
							cba = split_nod[i];
						}
					} else {
				...
				for (var i = 0; i < split_nod.length - 2; i++) {
					var jar = jarak[split_nod[i]][split_nod[i + 1]];
					tmp_jarak = parseFloat(tmp_jarak) + parseFloat(jar);
					var text_tmp = (i + 1) + ". <strong><span>" + nm_rs[split_nod[i + 1]] + " (" + parseFloat(tmp_jarak).toFixed(2) + " Km) </span></strong>";
					if (i + 1 == split_nod.length - 2) {
						cap = parseFloat(tmp_jarak) + parseFloat(jarak[split_nod[i + 1]][split_nod[i + 2]]);
						if (isNaN(cap)) {
							cap = tmp_jarak;
						}
					}
					if (nilai_des - 1 == split_nod[i + 1]) {
						text_tmp = "";
					}

					text_nod = text_nod + text_tmp + "<br>";

				}

				document.getElementById('jarak').innerHTML = "Jarak: <strong><span>" + parseFloat(cap).toFixed(2) + " Km</span></strong>";
				text1 = "Rekomendasi Terdekat : " + text_nod + "<br>Node: " + abc;
